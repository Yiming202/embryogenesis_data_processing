...
# PCA of division time using the same embryos and cell filters as
# pca_cell_cycle_length_selected_af16.R.
...
output_dir <- file.path(base_dir, "pca_cell_division_time")
...
process_one <- function(group, id, file_path, time_limit, minutes_per_tp, embryo) {
  read_csv(path.expand(file_path), show_col_types = FALSE) %>%
    ...
    group_by(cell) %>%
    summarise(division_time_min = max(time) * minutes_per_tp, .groups = "drop") %>%
    mutate(group, embryo)
}

time_long <- pmap_dfr(datasets_config, process_one)
...
observed_ids <- lineage %>% filter(cell %in% unique(time_long$cell)) %>% pull(lineage_id)
dividing_cells <- lineage %>%
  ...
  ungroup() %>%
  filter(fate != "Death", !cell %in% c("Zygote", "AB", "P1", "ABa", "ABp", "EMS", "P2"), has_observed_daughter) %>% pull(cell)

wide <- time_long %>% filter(cell %in% dividing_cells) %>% select(cell, embryo, division_time_min) %>% pivot_wider(names_from = embryo, values_from = division_time_min)%>%
  drop_na()
...
scores <- as.data.frame(pca$x[, 1:2]) %>% rownames_to_column("embryo") %>% left_join(datasets_config %>% select(embryo, group), by = "embryo")
...
p <- ggplot(scores, aes(PC1, PC2, colour = group)) +
  ...
  labs(title = "PCA of division-time profiles", subtitle = paste0(nrow(complete), " shared non-death dividing cells; selected AF16 six-embryo set"), x = sprintf("PC1 (%.1f%%)", variance[[1]]), y = sprintf("PC2 (%.1f%%)", variance[[2]])) +
...
write_csv(as.data.frame(complete) %>% rownames_to_column("cell"), file.path(source_dir, "PCA_division_time_matrix.csv"))
write_csv(scores, file.path(source_dir, "PCA_scores_division_time.csv"))
...
ggsave(file.path(figure_dir, "Figure_PCA_division_time_selected_AF16.pdf"), p, width = 6.7, height = 5.1)
ggsave(file.path(figure_dir, "Figure_PCA_division_time_selected_AF16.png"), p, width = 6.7, height = 5.1, dpi = 600)
